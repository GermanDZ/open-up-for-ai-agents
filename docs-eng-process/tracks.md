...
| `check-gates` | The script's `DEFAULT_REQUIRED_GATES` is the full set `team_deployed,log_written,roadmap_synced,implementation_verified` (used by `full`). **`quick` and `standard` are solo and call `check-gates --require log_written,roadmap_synced,implementation_verified`** — no team gate. Only `full` (or standard work that explicitly deployed a team) gates on `team_deployed`. `implementation_verified` (T-145, the delivery-evidence gate) is required on **every** track — the track relaxes ceremony, never evidence. |
