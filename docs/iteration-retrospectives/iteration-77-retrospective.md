...
| ~~Decide whether `gates.retro_due` should be checked outside `full`-track starts so a long solo `quick`/`standard` streak can't silently outrun the cadence again~~ | ~~project-manager / owner~~ | ~~next architecture pass~~ | **satisfied 2026-07-27 (T-151)** — decision recorded in `state-file.md`: the hard block stays `full`-only, **with an explicit revisit condition** (iterations 87–98 did outrun the cadence, but that streak was counted by a number since proven untrustworthy; a second occurrence is the clean evidence) |
| ~~Either exercise `/openup-fan-out` for real (two disjoint READY lanes, capture the wall-clock numbers T-060 specified) or explicitly retire/mark-n/a its success measure~~ | ~~owner~~ | ~~by iteration 90 or next retrospective~~ | **resolved 2026-07-27 (owner decision, T-154) — the measure is RETIRED as `n/a`.** The item offered two branches and the owner took the second. Rationale: 38 iterations since T-060 shipped, **zero** real two-lane uses, and sequential `/openup-next` is the established working default — a wall-clock comparison nobody will act on is not worth manufacturing. `/openup-fan-out` itself is untouched and remains available; only its success measure is retired. If fan-out is ever used in earnest, a fresh measure should be authored then, under T-152's read-back-environment rule |
| ~~Confirm whether `openup-agent.py cycle`'s judgment sub-runs are expected to write to `docs/agent-logs/runs/`; if yes, find why July's high-volume days under-logged relative to commit volume~~ | ~~owner~~ | ~~next iteration~~ | **satisfied 2026-07-27** — investigated; **there is no under-logging, the metric was invalid**. (a) Yes, the engine logs: `cycle.py` calls `openup-state.py log-event` and `_sweep_run_logs`. (b) On 2026-07-13 (the worst day: 167 commits, 90 records) **91 commits were logs-only** — deliberately skipped by the self-reference guard — and **27 were merges**, never logged; that leaves 49 loggable commits against 90 records, i.e. records *exceed* commits. (c) 17 of those 90 records point at SHAs unreachable from `main` (rebased away), while `git log` buckets by post-rebase committer date — so per-day record counts and per-day commit counts are not comparable quantities at all |
| ~~Decide if `docs/risk-list.md` is worth instantiating for this project's scale, or if the retrospective skill's step 4 should treat its absence as `n/a` rather than a silent skip~~ | ~~owner~~ | ~~next retrospective~~ | **satisfied 2026-07-27** (carried as `77.5`, 26 iterations) — owner decided **instantiate**, and `docs/risk-list.md` now exists (7 ranked risks, each citing the evidence it was identified from). The deciding argument was not this project's scale but a dangling promise: `docs-eng-process/getting-started.md`, `QUICK-REFERENCE.md`, `skills-guide.md`, `USER-GUIDE.md` and the `/openup-retrospective` skill all already referenced `docs/risk-list.md` as though it existed — the same defect class T-157 had just fixed in the conflict-recovery recipe. Step 4 therefore needs no `n/a` branch: the file it reads is there. Closed by **T-158** |
