...
|---|---|---|---|
| ~~**Clarify retro cadence gate boundary** — Should `retro_due` block on completion 4→5 (retro=4) or 5→6 (retro=5)?~~ | ~~PM (you)~~ | ~~HIGH~~ | **satisfied 2026-07-27 (T-151)** — decision recorded in `docs-eng-process/state-file.md` §"Recorded decisions (T-151)": the counter counts completions since the last retrospective, so the gate fires at `count >= 5`. No code change; the semantics are now written down where the gate is documented |
