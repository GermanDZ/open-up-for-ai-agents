...
| ID | Title | Status | Priority | Depends on |
|---|---|---|---|---|
| T-120 | Inline engine-held context into sub-run briefings (E1): judgment boxes get plan.md/design.md content inlined (engine already parsed plan.md), task-def instructions resolve workproduct-name inputs → concrete paths via the loaded library (small files inlined, size-capped + truncation marker), spec lanes get engine-read vision, assess gets a deterministic evidence bundle | in-progress | high | T-106 |
