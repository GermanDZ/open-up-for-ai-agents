...
| [T-114](changes/T-114/plan.md) | `/openup-init` template completeness — new `docs-eng-process/templates/project-status.md` + `roadmap.md`, and reference the existing `input-request.md` template for the stakeholder-brief step, instead of freehand heredoc authoring | completed (2026-07-14) | medium | — |
| [T-115](changes/T-115/plan.md) | Queryable Development Case archetype default + track/archetype disambiguation — a single command answers "what applies when `docs/project-config.yaml` is absent" instead of a 4-grep prose hunt; `tracks.md`/`project-config.md` name each other where "quick" is first introduced | completed (2026-07-14) | medium | — |
| [T-112](changes/T-112/plan.md) | New `/openup-cycle` skill — deterministic-first pick/resume entry point that classifies Operations boxes as script-vs-judgment (mirrors `cycle.py`'s `classify_box`), skipping self-brief ceremony on mechanical steps and gating before every tick; every other `resolve` path (plan-iteration/assess/milestone/replenish) routes to `/openup-next`, which stays byte-unchanged | completed (2026-07-14) | medium | — |
