...
- `retro_dir()` resolver + `--retro-dir` argument on the `retro` subparser —
  `scripts/openup-state.py`
- Worktree/migration tests in `scripts/tests/test_openup_state.py`
...
- [ ] Add `retro_dir()` (git-common-dir resolution with a non-git fallback to
      `REPO_ROOT/.openup`) and the `--retro-dir` argument; repoint
      `retro_path()` at it (`scripts/openup-state.py`)
- [ ] Add the read-forward legacy fallback in `read_retro_count()` so an
      existing `<worktree>/.openup/retro.json` seeds the shared value once
- [ ] Update the module docstring's `retro` subcommand line to name the new
      location
- [ ] Document location, override precedence, and migration in
      `docs-eng-process/state-file.md`
- [ ] (tester) Tests in `scripts/tests/test_openup_state.py`: shared path is
      used by default; a linked worktree reads/advances the same count;
      `--retro-dir` and `--state-dir` overrides win in that order; legacy value
      is carried forward once and not re-applied; non-git cwd falls back without
      raising
- [ ] Run the full test suite and `python3 scripts/check-docs.py`
