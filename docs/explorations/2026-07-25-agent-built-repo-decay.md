...
**blocked**: the analysis had no access to `kaze-webapp`, and the T-127 run could
...
|---|---|---|---|---|
| `kaze-webapp` | 2026-02-23 → 2026-07-25 (5 mo) | 379 | **97.1%** | is agent-built code fragile? |
| `tallyfox-app` | 2025-06-18 → 2026-07-24 (**13 mo**) | 1286 | 46.7% | does it decay *past* the 3–6 mo window? |

These are complementary, and better than the brief assumed. `kaze-webapp` is a
near-pure agent artifact — its 11 human commits are ops/config only (secrets,
domain, Kamal, SMTP), so nothing dilutes the signal. `tallyfox-app` runs **13
...
`tallyfox-app`, over 13 months and **14× growth** (7,926 → 110,464 lines):
...
`kaze-webapp` (97% agent) is flat on every structural measure across its life:
median 53 → 52, p90 168.5 → 169.4, share >400 lines 0.012 → 0.028. Its files
are dramatically *smaller* than tallyfox's human-era files (p90 169 vs 315).
...
- `tallyfox-app` files/commit by month: 4.78, 3.29, 3.73, 2.40, 2.54, 3.46,
...
- `kaze-webapp`: 9.63 → 5.69 → … → 7.16, with churn (deletions per line added)
  falling 0.269 → 0.05. The July rise follows a 3-month dormancy, not
  accumulation — which is also why kaze cannot test the 3–6 month window: it is
  5 months *calendar*, but only ~2.5 months active.

**tallyfox is the load-bearing repo for the thesis** — it is the only one of the
...
- `tallyfox-app`: 0.645 → 0.411 → 0.654 → 0.490. No trend over 13 months; the
  agent era is *lower* than the human era.
- `kaze-webapp`: 0.429 → 0.957 → 0.941 → 0.922 — high, but flat after the
  scaffolding burst.

Kaze's top pair is `config/locales/en.yml ~ config/locales/es.yml`, support 109,
...
`test/src` line ratio in `tallyfox-app` is **0.00 for the first seven human-built
months** (0.000, 0.000, 0.000, 0.010, 0.009, 0.007, 0.007), then jumps to 0.43
in 2026-02 and reaches 0.48 by 2026-07. `kaze-webapp` — 97% agent — is born at
...
- **T4 — vendored framework code is attributed to whoever ran the sync.**
  ...
  kaze's 168 `chore(process): sync OpenUP framework` commits are 19.5% of its
...
- **Option A — treat this as sufficient evidence to close the decay question.**
  ...
  agent vendor; kaze's dormancy means only tallyfox truly probes past 6 months.
- **Option B — productize the measures that changed the answer.** Fold
  ...
  conclusion on kaze via T4. Con: real delivery cost on a tool with one user.
...
- `kaze-webapp` has never been past ~2.5 active months. Re-measuring it after a
...
tool would have reported the opposite conclusion on `kaze-webapp` via trap T4
